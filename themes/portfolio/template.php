...
// Add Zen Tabs styles
if (theme_get_setting('portfolio_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'portfolio') .'/css/tabs.css');
}
...
function portfolio_preprocess_html(&$vars) {
  ...
  foreach ($user->roles as $role){
    $vars['classes_array'][] = 'role-' . portfolio_id_safe($role);
  }
  ...
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = portfolio_id_safe('page-'. $path);
    $vars['classes_array'][] = portfolio_id_safe('section-'. $section);
  ...
  // Add IE classes.
  if (theme_get_setting('portfolio_ie_enabled')) {
    $portfolio_ie_enabled_versions = theme_get_setting('portfolio_ie_enabled_versions');
    if (in_array('ie8', $portfolio_ie_enabled_versions, TRUE)) {
...
function portfolio_preprocess_page(&$vars, $hook) {
...
function portfolio_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from portfolio_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
function portfolio_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function portfolio_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('portfolio_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('portfolio_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('portfolio_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('portfolio_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('portfolio_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */
function portfolio_id_safe($string) {
 ...
 *
 */
function portfolio_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = portfolio_id_safe($element['#title']);
 ...
 */
function portfolio_preprocess_menu_local_task(&$variables) {
 ...
 */
function portfolio_menu_local_tasks(&$variables) {
  $output = '';
